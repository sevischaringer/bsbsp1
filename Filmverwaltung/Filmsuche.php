...
<?php
    ...
    else
    {
        ...
        else
        {
            ...
            <?php
            $query = 'select f.titel "Titel", f.erscheinungs_datum "Erscheinungs-Datum",pf.bezeichnung "Produktionsfirma" from produktionsfirma pf,film f where pf.produktionsfirma_id=f.Produktionsfirma_ID and pf.bezeichnung like"%"?"%" order by erscheinungs_datum';
            $stmt = $con->prepare($query);
            $stmt->bindParam(1, $produktionsfirma);
            $stmt->execute();
